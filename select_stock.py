...
def mystrategy2(data):
    ...
    rsv = (price.iloc[-1] - price.iloc[-150:].min()) / \
        (price.iloc[-150:].max() - price.iloc[-150:].min())

    condition1 = (市值 < 1e10)
    condition2 = 自由現金流 > 2000
    condition3 = 股東權益報酬率 > 0
    condition4 = 營業利益成長率 > 0.3
    condition5 = 市值營收比 < 3
    condition6 = rsv > 0.5
...
# 綜合上述
print(backtest(datetime.date(2019, 1, 1),
               datetime.date(2020, 12, 30), 60, mystrategy2, data))


# plt.show(backtest(datetime.date(2017, 1, 1),
#                   datetime.date(2018, 3, 10), 60, mystrategy, data))
